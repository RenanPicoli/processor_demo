...
*/
	...
	char* s0=calloc(MAX_STR_LENGTH,sizeof(char));//temporary string to store lines read from files
	char* s1=calloc(MAX_STR_LENGTH,sizeof(char));//temporary string to store lines read from files
	char* s2=calloc(MAX_STR_LENGTH,sizeof(char));//temporary string to store lines read from files
	char* s3=calloc(MAX_STR_LENGTH,sizeof(char));//temporary string to store lines read from files
	char* s4=calloc(MAX_STR_LENGTH,sizeof(char));//temporary string to store lines read from files
	char* s5=calloc(MAX_STR_LENGTH,sizeof(char));//temporary string to store lines read from files
	if(ptr==NULL){
	...
	while (!feof(fp_types)){
		...
		sscanf_retval=sscanf(tmp_str,"constant %[a-zA-Z0-9&\"_ ] : %*[a-zA-Z0-9&\"_() ] := \"%[01]\" ;",s0,s1);
		if(sscanf_retval>1){
			//printf("Opcode j=%d: %s := %s\n",j,s0,s1);
			dictionary = realloc(dictionary,(j+1)*sizeof(node));
			strncpy(dictionary[j].name,s0,11);
			strncpy(dictionary[j].binary_string,s1,7);
	...
	while (!feof(fp)){
		...
		//sscanf_retval=sscanf(tmp_str,"%d => %s & %s & %s & %s & %s & %s , %*s",&i,s0,s1,s2,s3,s4,s5);//reads a single line of fp, with no spaces, terminated with '\n', expects 8 hexadecimal digits (32bit data)
		if(sscanf_retval>1){
			//printf("Instrução i=%d: %s %s %s %s %s %s\n",i,s0,s1,s2,s3,s4,s5);
